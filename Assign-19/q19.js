...
function callFriends() {
    callFrens.forEach(function (name, i) {
        if (name === notInTheCity) {
            callFrens[i] = "Ayan";
            console.log("Hi ".concat(name, " i would like to dinner with you thats why i'm inviting you"));
        }
        else {
            console.log("Hi ".concat(name, " i would like to dinner with you thats why i'm inviting you"));
        }
...
for (var i = array_length; i > 2; i--) {
    console.log("Sorry ".concat(callFrens[i - 1], " you are not invited because of space issue"));
    callFrens.pop();
}
